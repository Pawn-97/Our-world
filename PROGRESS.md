...
6. **封面由脚本自动挑选**：取最近一次到访中「像素最多且偏横构图」的那张，偶尔会选中食物照。人工调整走编辑器（`coverMediaByPlace`）或直接改 `content/places.json` 的 `coverMediaId`。
...
2. **封面人工挑选**：13 个已到访地点的封面目前是脚本自动选的，值得逐个过一遍（尤其上海那张现在是食物照）。
